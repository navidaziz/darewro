...
class Public_Controller extends MY_Controller
{
	...
	public function __construct()
	{
		...
		$where = "`menu_pages`.`status` IN (1) ORDER BY `menu_pages`.`order`";
		$this->data["menu_pages"] = $this->menu_page_model->get_menu_page_list($where, FALSE);

		foreach ($this->data["menu_pages"] as $menu_page) {
			$where = "`menu_sub_pages`.`status` IN (1) AND `menu_sub_pages`.`menu_page_id` =" . $menu_page->menu_page_id . " ORDER BY `menu_sub_pages`.`order`";
