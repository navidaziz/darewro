...
class MY_Controller extends CI_Controller
{
	...
	function rider_push_notification($rider_id)
	{
		...
		$fields = array(
			'registration_ids' 	=> array($rider_order_detail->mobile_token),
			'data'			=> $msg
		);
		$headers = array(
	...
	function restaurant_push_notification($restaurant_id)
	{
		...
		$fields = array(
			'registration_ids' 	=> array($user_order_detail->mobile_token),
			'data'			=> $msg
		);
		$headers = array(
			'Authorization: key=' . API_ACCESS_KEY,
	...
	function send_sms($to, $from, $message)
	{
		...
		$result = $client->QuickSMS(array('obj_QuickSMS' => array(
			'loginId' => $user_name,
	...
	public function upload_file($field_name, $config = NULL)
	{
		if (is_null($config)) {
			$config = array(
				...
				"allowed_types" => "jpg|jpeg|bmp|png|svg",
				"max_size" => 10000,
				"max_width" => 0,
				"max_height" => 0,
				"remove_spaces" => true,
